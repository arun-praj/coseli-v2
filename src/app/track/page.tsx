...
import { Search, Package, CheckCircle2, Truck, AlertCircle, XCircle } from "lucide-react";
                        ...
                        <div className="border-t border-zinc-200 pt-8 pb-4">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    ...
                                    <span className="text-2xl font-serif text-black capitalize">{orderData.status}</span>
                                </div>
                            </div>
                            ...
                            {orderData.status === 'cancelled' ? (
                                <>
                                    ...
                                    <div className="flex justify-between items-center text-[10px] font-bold tracking-widest uppercase mt-3 max-w-sm mx-auto">
                                ...
                                <>
                                    <div className="mt-8 flex items-center w-full max-w-sm mx-auto">
                                        <div className={`h-1 flex-1 ${['pending', 'fulfilled', 'shipped', 'delivered'].includes(orderData.status) ? 'bg-black' : 'bg-zinc-200'}`} />
                                        <div className={`w-3 h-3 rounded-full ${['pending', 'fulfilled', 'shipped', 'delivered'].includes(orderData.status) ? 'bg-black' : 'bg-zinc-200'}`} />

                                        <div className={`h-1 flex-1 ${['fulfilled', 'shipped', 'delivered'].includes(orderData.status) ? 'bg-black' : 'bg-zinc-200'}`} />
                                        <div className={`w-3 h-3 rounded-full ${['fulfilled', 'shipped', 'delivered'].includes(orderData.status) ? 'bg-black' : 'bg-zinc-200'}`} />

                                        <div className={`h-1 flex-1 ${['shipped', 'delivered'].includes(orderData.status) ? 'bg-black' : 'bg-zinc-200'}`} />
                                        <div className={`w-3 h-3 rounded-full ${['shipped', 'delivered'].includes(orderData.status) ? 'bg-black' : 'bg-zinc-200'}`} />

                                        <div className={`h-1 flex-1 ${orderData.status === 'delivered' ? 'bg-black' : 'bg-zinc-200'}`} />
                                        <div className={`w-3 h-3 rounded-full ${orderData.status === 'delivered' ? 'bg-black' : 'bg-zinc-200'}`} />
                                    </div>
                                    <div className="flex justify-between items-center text-[10px] font-bold tracking-widest uppercase text-zinc-400 mt-3 max-w-sm mx-auto">
                                        <span className={['pending', 'fulfilled', 'shipped', 'delivered'].includes(orderData.status) ? 'text-black' : ''}>Pending</span>
                                        <span className={['fulfilled', 'shipped', 'delivered'].includes(orderData.status) ? 'text-black' : ''}>Processing</span>
                                        <span className={['shipped', 'delivered'].includes(orderData.status) ? 'text-black' : ''}>Shipped</span>
                                        <span className={orderData.status === 'delivered' ? 'text-black' : ''}>Delivered</span>
                                    </div>
                                </>
                            )}
